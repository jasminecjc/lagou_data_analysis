...
db = MySQLdb.connect(host="localhost", user="root", passwd="55556666", db="graduate_work", charset="utf8")
cursor = db.cursor()
...
def valid_proxy(path, method, code = 0, *payload):
    while code != 200:
        try:  
            ...
            if method == 'get':     
                source = session.get(path, headers = headers, proxies = proxies, timeout = 20)
            else:
                source = session.post(path, headers = headers, proxies = proxies, data = payload[0], timeout = 20) 
            try:
...
# 公司分析
def companys():    
    path = 'https://www.lagou.com/gongsi'
    source = partial(valid_proxy, path, 'get', 0)()[0]
