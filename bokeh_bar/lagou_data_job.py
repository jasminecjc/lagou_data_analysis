...
job_options = ['All', u'数据挖掘',u'数据可视化', u'数据分析', u'大数据工程师', u'数据架构师']

job = Select(title=u"岗位选择", value=u"All", options=job_options)
...
x_axis = Select(title=u"分析纬度",
                ...
                value=u"职位所在城市") 
get_city_sql = 'select city, count(*) as count from lagou_data_job group by city order by count desc limit 0,15'
...
p = Bar(data, values='count', label='city', agg='mean',
        title=u"按城市维度分析", legend='top_right', width=600,toolbar_location="left")
